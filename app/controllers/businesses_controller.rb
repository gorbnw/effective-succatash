class BusinessesController < ApplicationController
  ...
  def show
    ...
    @positive_testimonial = @testimonials.where(positive: true).reduce do |top_testimonial, testimonial|
      if testimonial.votes.count > top_testimonial.votes.count
        top_testimonial = testimonial
      end
      top_testimonial
    end
    @negative_testimonial = @testimonials.where(positive: false).reduce do |top_testimonial, testimonial|
      if testimonial.votes.count > top_testimonial.votes.count
        negative_testimonial = testimonial
      end
